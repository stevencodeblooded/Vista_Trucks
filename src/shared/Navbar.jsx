      ...
      <section className="max-w-4xl mx-auto">
        <nav className='flex items-center justify-between gap-5 uppercase text-white'>
          ...
          <ul className='hidden sm:flex items-center gap-8 text-xs'>
            ...
            <li>
              <NavLink to={'contacts'} className={({ isActive }) => isActive ? 'border-b-2 border-b-red-600 pb-1' : 'transition-all hover:border-b-2 hover:border-b-red-600 hover:pb-1'}>Contacts</NavLink>
            </li>
          </ul>
          <div className='flex items-center gap-5'>
            <Link to={'contact'} className='bg-red-600 hover:bg-red-700 capitalize transition-all sm:px-6 px-4 py-1 sm:py-2 rounded-md text-white font-semibold'>Contact Us</Link>
            <div className='sm:hidden text-2xl'>
              <FontAwesomeIcon icon={faBars} className='cursor-pointer' onClick={() => setMobileLinks(!mobileLinks)} />
            </div>
          </div>
        </nav>
        <div className={`mobile-links ${mobileLinks ? 'show' : ''}`}>
